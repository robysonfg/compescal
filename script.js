...
document.getElementById('form-login').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (data.status === 'sucesso') {
            usuarioLogado = data.dados;
            document.getElementById('user-name').textContent = usuarioLogado.nome;
            document.getElementById('view-login').classList.add('hidden');
            document.getElementById('view-app').classList.remove('hidden');
            
            if(usuarioLogado.perfil.toUpperCase() === 'LIDER') iniciarLider();
            else if(usuarioLogado.perfil.toUpperCase() === 'PORTARIA') iniciarPortaria();
            else if(usuarioLogado.perfil.toUpperCase() === 'RH') iniciarRH();
...
window.iniciarFormulario = function(tipo) {
    ...
    if(tipo === 'Entrada') {
        boxRetorno.classList.add('hidden');
        document.getElementById('vai-retornar').value = 'Não';
    } else {
        boxRetorno.classList.remove('hidden');
    }
...
function togglePrevisaoRetorno() {
    const select = document.getElementById('vai-retornar').value;
    const box = document.getElementById('box-prev-retorno');
    const input = document.getElementById('prev-retorno');
    if(select === 'Sim' && direcaoAtual === 'Saída') { 
        box.classList.remove('hidden'); input.required = true; input.value = dataHoraInputLocal(); 
    } else { 
        box.classList.add('hidden'); input.required = false; input.value = '';
    }
}
...
document.getElementById('form-autorizacao').addEventListener('submit', async (e) => {
    ...
    const dados = [
        ...
        document.getElementById('mat-colaborador').value,
        nome,
    ...
    try {
        ...
        const data = await res.json();
        if(data.status === 'sucesso') { 
            showToast("Autorização enviada!", "sucesso"); 
            voltarSelecao();
        } else { showToast(data.mensagem, "erro"); }
    ...
    try {
        ...
        autorizacoes.reverse().forEach(auth => {
            ...
            if (auth.Status === "Aguardando Liberação de Saída" || auth.Status === "Aguardando Portaria") {
                htmlSaida += `
                    <div class="auth-card" style="border-left-color: var(--accent);">
                        <div style="display:flex; justify-content:space-between;"><div class="name">${auth.Nome}</div> ${vaiRetornarTag}</div>
                        ${detalhesComuns}
                        <div class="details"><strong>Prev. Saída:</strong> <span class="text-danger">${pAcaoBR}</span></div>
                        <button onclick="acionarPortaria('${auth.ID}', '${auth.Nome}', 'saida')" class="btn btn-danger mt-1"><i class="ph ph-sign-out"></i> Confirmar Saída</button>
                    </div>`;
            } 
            // AGUARDANDO ENTRADA
            else if (auth.Status === "Aguardando Liberação de Entrada") {
                htmlEntrada += `
                    <div class="auth-card" style="border-left-color: var(--primary);">
            ...
            // AGUARDANDO RETORNO
                ...
                htmlRetorno += `
                    <div class="auth-card" style="border-left-color: var(--warning);">
                        <div class="name">${auth.Nome}</div>
                        ${detalhesComuns}
                        <div class="details"><strong>Saiu às:</strong> ${acaoRealBR}</div>
            ...
            // HISTÓRICO DE HOJE
                ...
                htmlHistorico += `
                    <div class="auth-card ${classeCor}">
                        <div style="display:flex; justify-content:space-between;"><div class="name">${auth.Nome}</div><span style="font-size:0.7rem; font-weight:bold; color:var(--text-light);">${icone}</span></div>
...
window.acionarPortaria = async function(id, nome, acao) {
    ...
    try {
        ...
        const data = await res.json();
        if(data.status === 'sucesso') { showToast("Registrado!", "sucesso"); carregarPortaria(); } 
        else { showToast(data.mensagem, "erro"); }
    } catch(e) { showToast("Falha na conexão.", "erro"); }
}
...
window.aplicarFiltrosRH = function() {
    ...
    if (tipo === 'lider') {
        ...
        renderizarTabelaRankingRH(dadosFiltrados);
    } 
    else {
        renderizarTabelaGeralRH(dadosFiltrados);
    }
...
window.exportarPDF = function() {
    ...
    doc.autoTable({
        html: '#tabela-relatorio', startY: 30, theme: 'striped',
        headStyles: { fillColor: [15, 44, 89] }, styles: { fontSize: 8 }
    });
